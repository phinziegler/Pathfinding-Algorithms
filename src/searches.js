...
class Search {
    ...
    breadthFirst(tileArray) {
        ...
        while(!frontier.isEmpty()) {
            ...
            if(v.isGoal()) {
                this.colorPath(v);
                console.log("complete, found solution");
                return;
            }
    ...
    colorPath(tile) {
        ...
        let n = 0;
        while(current.getParent() != null && n < 200) {
            n++;
            console.log(current.id);
            current.setColor(this.solutionCols[0]);
            current.setBorderColor(this.solutionCols[1]);
